...
import { projectColors } from "@/lib/constants/projectColors";
...
const Dashboard: NextPage = (
    ...
    const getProjects = useCallback(async () => {
        try {
            const QUERY = `
        query GetProjectsByUserID($userId: ID, $orderBy: ProjectOrderByInput) {
            getProjectsByUserID(userId: $userId, orderBy: $orderBy) {
                name
                key
                description
                owner {
    ...
    return (
        <div className={cn(
            ...
            <div className="w-full grid grid-cols-1 lg:grid-cols-[70%_30%]">
                <div>
                        ...
                        <div className="w-full px-4 py-6">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <Button variant="ghost" asChild>
                                    <Link href={`/dashboard/${session?.user?.fdlst_private_userId}/manageprojects`} className="flex items-center gap-1">
                                        View All Projects
                            ...
                            {loading ? (
                                ...
                                <div className={cn("grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-fit gap-2",
                                )}>
                                    {projects.slice(0, 3).map((project) => (
                                        <Card
                                            key={project._id}
                                            className={cn(
                                                `hover:shadow-lg transition-shadow group rounded-sm w-[256px]`,
                                                `border border-l-[12px]`,
                                            )
                                        }
                                        style={{ borderLeftColor: projectColors[Math.floor(Math.random() * projectColors.length)] }}
                                        >
                                            <Link
        ...
        // <div
        //     className={cn(
        //         "flex flex-col justify-center items-center gap-2 overflow-y-clip px-2 transition-all duration-300 ease-in-out mt-navbar-offset text-neutral-900",
        //         open ? "w-screen-minus-sidebar" : "w-screen-minus-sidebar-icons",
        //         robotoCondensed.className
        //     )}
        // >
